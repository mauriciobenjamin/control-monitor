 ...
 ************************************************/
  ...
  */
  ...
  void controlReg() {
    ...
    while (ciclos >= contCiclos || ciclos == 0) {
      ...
      for (size_t i = 0; i < etapas; i++) {
        ...
        } else if (!para_etapas[i].usaLz() && relLuz.edoRel()) {
        ...
        while(regresivo > 0) {
          ...
          if(alarmaAct == false) {
            sigAlarmSeg = rtc.getSeconds() - 1; //Por alguna obscura razón se gana un segundo
  ...
  void datosPantalla() {
    switch (opDatos) {
      case hyT:
        ...
        if (retiene + 2900 < millis()) {
          opDatos = lUv;
        }
        break;
      case lUv:
        if (retiene + 5900 < millis()) {
          opDatos = cuenta;
        }
        if (inicial == 2) {
          luzPantalla();
      ...
      case cuenta:
        ...
        if (retiene + 8900 < millis()) {
          opDatos = hhmm;
        }
        break;
      case hhmm:
        muestraTiempo();
        if (retiene + 11700 < millis()) {
          opDatos = hyT;
          inicial = 0;
        }
        break;
  ...
  void regresivoPantalla() {
    ...
    dosDigitos(falta); lcd.print(":"); dosDigitos((falta%4)*15);
